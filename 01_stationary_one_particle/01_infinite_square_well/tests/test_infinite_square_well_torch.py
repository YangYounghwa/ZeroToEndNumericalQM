import infinite_square_well_numpy_solution
import infinite_square_well_torch_solution
import numpy as np
import torch


def test_torch_hamiltonian_is_hermitian() -> None:
    _, _, hamiltonian = infinite_square_well_torch_solution.build_hamiltonian(
        num_points=40
    )
...
def test_torch_wavefunctions_are_discretely_orthonormal() -> None:
    result = infinite_square_well_torch_solution.solve_infinite_well(
...
def test_numpy_and_torch_energies_agree() -> None:
    numpy_result = infinite_square_well_numpy_solution.solve_infinite_well(
        num_points=80, num_states=4
    )
    torch_result = infinite_square_well_torch_solution.solve_infinite_well(
        num_points=80, num_states=4
    )
