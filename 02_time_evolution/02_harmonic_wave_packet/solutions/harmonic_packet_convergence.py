...
def analytical_packet(grid: torch.Tensor, time: float) -> torch.Tensor:
    """Sample the coherent state up to a global phase, in m = hbar = omega = 1."""
    phase = torch.tensor(time, dtype=torch.float64, device=grid.device)
    center = 2.0 * torch.cos(phase) + 0.7 * torch.sin(phase)
    wave_number = 0.7 * torch.cos(phase) - 2.0 * torch.sin(phase)
    state: torch.Tensor = torch.pi ** (-0.25) * torch.exp(
        -0.5 * (grid - center) ** 2 + 1j * wave_number * (grid - center)
    )
    return state
...
def grid_convergence() -> None:
    ...
    for points in (79, 159, 239):
        grid, spacing = solution.make_grid(points, -8.0, 8.0)
        hamiltonian = solution.build_hamiltonian(grid, spacing)[1]
        initial = solution.coherent_state(grid)
        numerical = solution.matrix_exponential_state(
            initial, hamiltonian, spacing, final_time
        )
        exact = analytical_packet(grid, final_time)
